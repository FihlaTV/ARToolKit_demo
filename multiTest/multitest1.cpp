...
static void mainLoop(void)
{
	...
	for (i = 0; i < config->marker_num; i++) {
		...
		//if (config->marker[i].visible >= 0) draw(config->trans, config->marker[i].trans, 0);//蓝色 可见
		//else                                 draw(config->trans, config->marker[i].trans, 1);//红色 不可见
	}
	argSwapBuffers();
}
static void draw(double trans1[3][4], double trans2[3][4], int mode)
